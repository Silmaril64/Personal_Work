...
__circuit__ = ((10,10),(50,50),(75,150),(200, 170),(550,40), (620,10),(500,200),(650,300),(620, 600),(200,150))
__screenSize__ = (1000,1000)
...
class Vehicule:
    ...
    def steerUpdate(self, track, vehicules):
        ...
        self._force = vecAdd(self._force, self.steerSeparation(vehicules._vehicules))
    ...
    def steerPathFollow(self, track):
        ...
        (sf, futurePosition, toNext) = track._segmentPointAddLength(s, p, max(10,approximateLength(self._speed)) * self._seeInFuture) 
        # We just have to register a force to get to futurePosition !
        force = vecDiff(futurePosition, self._coords)
        tempo_length = approximateLength(force)
        
        #tempo_length = approximateLength(vecAdd(self._speed, force))
        #if (toNext != None and toNext < tempo_length):
        #    force = vecScalarMult(force, 0.5) #Un facteur inversement proportionnel
        unary_vec = (track._circuit[s][0] - track._circuit[s-1][0],track._circuit[s][1] - track._circuit[s-1][1])
        tempo_length = approximateLength(unary_vec)
        unary_vec = [unary_vec[0] / tempo_length, unary_vec[1] / tempo_length]
        dot = vecDot(self._speed,  unary_vec)
        if (toNext != None and  dot > toNext):
            unary_vec = [- unary_vec[0] * dot, -unary_vec[1] * dot]
            force = vecAdd(force, unary_vec )
            
        if (tempo_length > self._maxforce):
            force = vecScalarMult(force,self._maxforce/tempo_length)
        return force

    def steerSeparation(self, vehicules):
        forceAccu = (0,0) # starts with a fresh force
        for v in vehicules: #TODO: utiliser le tree (ou directement la lib ANN)
            if v is not self:
                tempo = [0,0]
                if (self._coords[0] - v._coords[0] > 0.01):
                    tempo[0] = self._baseseparationforce /(self._coords[0] - v._coords[0])**2
                else:
                    tempo[0] = 0.01
                if (self._coords[1] - v._coords[1] > 0.01):
                    tempo[1] = self._baseseparationforce / (self._coords[1] - v._coords[1] )**2
                else:
                    tempo[1] = 0.01
                forceAccu = (forceAccu[0] + tempo[0], forceAccu[1] + tempo[1])
        #print(forceAccu)
        return forceAccu
...
class SetOfVehicules:
    ...
    def updatePositions(self):
        for v in self._vehicules:
            ...
            v._coords= (v._coords[0]+v._speed[0], v._coords[1]+v._speed[1])
...
class Track:
    ...
    def _segmentPointAddLength(self, segment, point, length):
        ...
        if nextStep > length: # We stay on the same segment
            ...
            return (segment, (int(nextPoint[0]), int(nextPoint[1])), None)
        length -= nextStep
        ...
        return (segment, (int(nextPoint[0]), int(nextPoint[1])), nextStep)
